...
> **V2.1.0 配色更新：** 全站主色調改為**薰衣草紫系**（因應 Dora Study app logo、國中女生向），由 SELA 主動要求。原北歐暖米色票已整體位移到薰衣草霧底 + 矢車菊紫/蘭花/薄荷/玫瑰 accent，下列色碼已同步更新。設計鐵律（無漸層/陰影/大圓角、字重 300/400/500）不變。
...
--bg:       #F6F3FC;   /* 頁面背景：暖羊皮紙 */
--surface:  #FFFFFF;   /* 卡片白 */
--surface2: #F1ECFA;   /* 微暖白，用於 hover 或嵌套區域 */

--border:       #E8E1F2;   /* 常用邊框 */
--border-mid:   #CBC0E0;   /* 較深邊框，分隔線、箭頭 */
--border-dark:  #A89EC2;   /* 最深邊框，少用 */

--text-1: #2A2335;   /* 主文字：近黑色 */
--text-2: #6E6580;   /* 次要文字 */
--text-3: #A89FBC;   /* 說明文字、placeholder */
```

### 三個科目的 accent 色（自然材質靈感）

```css
--math:        #5B57C2;   --math-light:  #ECEBFA;   /* 深海板岩藍 */
--phys:        #4DA38E;   --phys-light:  #E6F4EF;   /* 森林綠 */
--chem:        #A368B0;   --chem-light:  #F4E9F6;   /* 暖棕土/赤陶 */
```

### 狀態色

```css
--done:    #4DA38E;   --done-bg:  #E6F4EF;   /* 完成：森林綠 */
--wip:     #B98A3E;   --wip-bg:   #F7EFDD;   /* 進行中：麥穗黃 */
--todo:    #A89FBC;   --todo-bg:  #EFEAF7;   /* 待辦：灰 */
```
...
|---|---|---|---|---|
| 觀念判斷 | `--concept-c` | `#B98A3E` | `#F7EFDD` | `#C9A86A` |
| 平衡方程式 | `--balance-c` | `#4DA38E` | `#E6F4EF` | `#8FC9A8` |
| 質量守恆 | `--mass-c` | `#A368B0` | `#F4E9F6` | `#C99AC0` |
| 莫耳計算 | `--mole-c` | `#5B57C2` | `#ECEBFA` | `#7A67C7` |
| 手寫題 | `--written-c` | `#7A569E` | `#F0E9F8` | `#9A77C7` |
...
```css
--c1: #5B57C2;  --c1l: #ECEBFA;   /* 6-1 原子說：板岩藍 */
--c2: #4DA38E;  --c2l: #E6F4EF;   /* 6-2 分子：森林綠 */
--c3: #B98A3E;  --c3l: #F7EFDD;   /* 6-3 粒子觀點：麥穗黃 */
--c4: #7A569E;  --c4l: #F0E9F8;   /* 6-4 化學式：煙燻紫 */
--c5: #A368B0;  --c5l: #F4E9F6;   /* 6-5 原子量莫耳：赤陶土 */
--c6: #5E83C0;  --c6l: #E9EFFA;   /* 章末複習：北歐藍綠 */
```
...
**總結區（Summary）**
```css
background: #E6F4EF; border: 1px solid #8FC9A8;
...
- **Active/Press**：background → #EDE7F6（比 surface2 更深一點）
...
- [ ] `<meta name="theme-color" content="#F6F3FC">` 已加入
